...
public class LogisticsViewModel extends AndroidViewModel {
    ...
    private void fetchPlannedDaysForUser(String userId) {
        databaseManager.getDestinationsReference().
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        for (DataSnapshot destination : snapshot.getChildren()) {
                            ...
                            if (userId.equals(username)) {
                                Integer duration = destination.child(
                                        "daysPlanned").getValue(Integer.class);
                                if (duration != null) {
    ...
    private void checkCollaboratorStatusAndFetch(String currentUsername,
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        ...
                        if (isCollaborator != null && isCollaborator) {
                            databaseManager.getUsersReference().
                                    child(currentUsername).child("mainUserId")
                                    .addListenerForSingleValueEvent(new ValueEventListener() {
                                        @Override
                                        public void onDataChange(@NonNull DataSnapshot snapshot) {
    ...
    public void inviteUser(String username, String currentUsername) {
        ...
        databaseManager.getUsersReference().child(username)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        if (snapshot.exists()) {
                            Boolean isCollaborator = snapshot.child(
                                    "isCollaborator").getValue(Boolean.class);
                            if (Boolean.TRUE.equals(isCollaborator)) {
                                inviteStatus.setValue("User "
                                        + username
                                        + " is already a collaborator and cannot be added again.");
                            } else {
                                databaseManager.addCollaborator(username, currentUsername,
                                        aVoid -> {
                                            ArrayList<String> updatedContributors =
                                        ...
                                        e -> inviteStatus.setValue("Failed to add collaborator: "
                                                + e.getMessage())
                                );
                            }
                        } else {
    ...
    private void checkCollaboratorStatusAndFetchContributors(String currentUsername) {
        databaseManager.getUsersReference().child(currentUsername).child("isCollaborator")
                .addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            databaseManager.getUsersReference().
                                    ...
                                    .addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        public void onDataChange(@NonNull DataSnapshot
                                            ...
                                            if (mainUserId != null) {
                                                ArrayList<String> loadedContributors =
                                                        new ArrayList<>();
                                                loadedContributors.add(mainUserId);
                                                ...
                                                fetchAdditionalContributors(mainUserId,
                                                        currentUsername, loadedContributors);
                                            }
                                        }

                                        @Override
                                        public void onCancelled(@NonNull DatabaseError error) {
                                            inviteStatus.setValue("Failed to fetch main user ID: "
                                                    + error.getMessage());
                                        }
                                    });
                        } else {
                    ...
                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        inviteStatus.setValue("Failed to check collaborator status: "
                                + error.getMessage());
                    }
                });
    }

    private void fetchAdditionalContributors(String userId, String currentUsername,
                                             ArrayList<String> loadedContributors) {
                ...
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot contributorSnapshot : snapshot.getChildren()) {
                            ...
                            if (contributor != null && !contributor.equals(currentUsername)
                                    && !loadedContributors.contains(contributor)) {
    ...
    public void addNoteForCurrentUser(String note) {
        ...
        databaseManager.getUsersReference().child(username).child("notes").setValue(notes)
                ...
                .addOnFailureListener(e -> noteStatus.setValue("Failed to add note: "
                        + e.getMessage()));
    }
}
